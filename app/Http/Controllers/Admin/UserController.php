...
class UserController extends Controller
{
    ...
    public function add()
    {
        ...
        $formUrl = route('common', ['class' => 'user', 'action' => 'add']);
    ...
    private function saveUserInfoToDatabase()
    {
        ...
        if($manager->addUser($data)) return Js::locate(route('common', ['class' => 'user', 'action' => 'index']), 'parent');
    ...
    public function edit()
    {
        ...
        $groupList = $groupModel->getGroupLevelLessThenCurrentUser($groupInfo['level']);
        $formUrl = route('common', ['class' => 'user', 'action' => 'edit']);
    ...
    private function updateUserInfoToDatabase()
    {
        ...
        $manager = new UserActionProcess();
        if($manager->editUser($data)) return Js::locate(route('common', ['class' => 'user', 'action' => 'index']), 'parent');
        return Js::error($manager->getErrorMessage());
    }
